...
```bash
curl -X POST https://api.ctxfy.com/api/v1/spaces \
...
```bash
curl -X POST https://api.ctxfy.com/api/v1/spaces/{spaceId}/collections \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://api.ctxfy.com/api/v1/spaces/{spaceId}/sources \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "type": "CONTENT",
...
```bash
curl -X POST https://api.ctxfy.com/api/v1/upload/s3 \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl https://api.ctxfy.com/api/v1/sources/{sourceId} \
  -H "Authorization: Bearer YOUR_API_KEY"
```
